jQuery(document).ready(function() {
	...
	if(jQuery('.mbt_help #mbt_selected_tutorial_video').length > 0) {
		const video_id = jQuery('.mbt_help #mbt_selected_tutorial_video').val().replace(/[^a-zA-Z0-9_-]/, '');
		jQuery('.mbt_help').find('#mbt_video_'+video_id).show();
		...
		jQuery.post(ajaxurl,
			{
				...
				mbt_nonce: jQuery(this).attr('data-nonce'),
			},
			function(response) {
				jQuery('.mbt-check-reviews-checking').hide();
				jQuery('.mbt-check-reviews-begin').show();
...
function mbt_do_feedback_refresh(element) {
	if(!element.attr('disabled')) {
		...
		if(element.attr('type') == 'radio') {
			jQuery('input[name='+element.attr('name')+']').attr('disabled', 'disabled');
		}
		...
		if(element.attr('data-element') === 'self') {
			...
			var elements = element.attr('data-element').split(",");
			data = {};
		...
		jQuery.post(ajaxurl,
			{
				...
				data: data,
				mbt_nonce: element.attr('data-refresh-nonce'),
			},
			function(response) {
