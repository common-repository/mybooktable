...
function mbt_add_sort_books_link($views) {
	// <a href="'.mbt_get_custom_page_url('mbt_sort_books').'">'.__('Sort Books', 'mybooktable').'</a>
	$link = wp_nonce_url(mbt_get_custom_page_url('mbt_sort_books'),'nonce_mbt','mbt_nonce');
	$views['sorting'] = '<a href="'.$link.'">'.__('Sort Books', 'mybooktable').'</a>';
	return $views;
}
...
function mbt_render_sort_books_page() {
	...
	if(!isset($_REQUEST['mbt_nonce']) || !wp_verify_nonce(sanitize_key($_REQUEST['mbt_nonce']), 'nonce_mbt')) { return; }
	if(!empty($_REQUEST['book_order']) && !empty($_REQUEST['mbt_manual_book_order']) && $_REQUEST['book_order'] == 'manual') {
		$data = json_decode(str_replace('\"', '"', sanitize_text_field(wp_unslash($_REQUEST['mbt_manual_book_order']))));
	...
	if(!empty($_REQUEST['book_order']) &&  isset($_REQUEST['mbt_book_sorting_attributes']) && $_REQUEST['book_order'] == 'attribute') {
		$data = json_decode(str_replace('\"', '"', sanitize_text_field(wp_unslash($_REQUEST['mbt_book_sorting_attributes']))));
		...
		mbt_update_setting('book_sorting_attributes', sanitize_text_field(wp_unslash($_REQUEST['mbt_book_sorting_attributes'])));
	}
	$book_sorting_attributes = mbt_get_setting('book_sorting_attributes');

	?>
	<div class="wrap mbt_sort_books">
		<div id="icon-options-general" class="icon32"><br></div><h2><?php esc_attr_e('Sort Books', 'mybooktable'); ?></h2>

		<div class="mbt_sorting_box">
			<div class="mbt_sorting_box_title"><?php esc_attr_e('Attribute Sorting', 'mybooktable'); ?></div>
			<div class="mbt_sorting_box_content">
				<form id="mbt_sort_books_form" method="post" action="<?php echo(esc_url(admin_url('admin.php?page=mbt_sort_books&book_order=attribute'))); ?>">
					<input id="mbt_book_sorting_attributes" name="mbt_book_sorting_attributes" type="hidden" value="<?php echo(wp_kses_post(htmlentities(str_replace('\"', '"', $book_sorting_attributes)))); ?>">
					<div id="mbt_book_attributes">
						<div id="mbt_book_attribute_adder"><span class="dashicons dashicons-plus"></span></div>
					</div>
					<input type="submit" name="save_settings" class="button button-primary" value="<?php esc_attr_e('Sort', 'mybooktable'); ?>" onclick="return mbt_submit_book_sorting_attributes();">
				</form>
			</div>
		</div>

		<div class="mbt_sorting_box">
			<div class="mbt_sorting_box_title"><?php esc_attr_e('Manual Sorting', 'mybooktable'); ?></div>
			<div class="mbt_sorting_box_content">
				<form id="mbt_sort_books_form" method="post" action="<?php echo(esc_url(admin_url('admin.php?page=mbt_sort_books&book_order=manual'))); ?>">
					<?php
						wp_nonce_field('nonce_mbt','mbt_nonce');
						$query = new WP_Query(array('post_type' => 'mbt_book', 'orderby' => 'menu_order', 'posts_per_page' => 300));
						if($query->have_posts()) {
							if(count($query->posts) > 300) {
								echo('<div class="mbt_sorting_error">'.esc_attr_e('Manual book sorting is not supported for more than 300 books. Please use Attribute Sorting.', 'mybooktable').'</div>');
							} else {
								echo('<input type="submit" name="save_settings" class="button button-primary" value="'.sprintf(esc_html__('Save Changes', 'mybooktable')).'" onclick="return mbt_submit_manual_book_order();">');
								echo('<input id="mbt_manual_book_order" name="mbt_manual_book_order" type="hidden" value="">');
								echo('<ul id="mbt_manual_book_sorter">');
								while($query->have_posts()) {
									$query->the_post();
									echo('<li data-id="'.esc_attr($post->ID).'" class="mbt_book">'.esc_attr($post->post_title).'</li>');
								}
								echo('</ul>');
							}
						} else {
							esc_attr_e('No books to sort!', 'mybooktable');
						}
					?>
				</form>
			</div>
