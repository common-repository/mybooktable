...
function mbt_parse_custom_importer_form() {
	if( isset($_REQUEST['data-import-nonce']) && isset($_FILES['mbt_custom_import_file']['tmp_name']) && wp_verify_nonce(sanitize_key($_REQUEST['data-import-nonce']),'mbt-import-nonce')) {
		...
		$arg = ''; // no idea why this needs to exist -- Come-on WP - make sense
		$filesystem = new WP_Filesystem_Direct($arg);
		$file_text = $filesystem->get_contents($file);
		...
		// load taxonomies
		foreach(array('mbt_author' => 'authors', 'mbt_series' => 'series', 'mbt_genre' => 'genres', 'mbt_tag' => 'tags') as $tax_slug => $tax_name) {
...
function mbt_detect_custom_export_download() {
	...
	$import_type = filter_input( INPUT_GET, 'mbt_import_type', FILTER_UNSAFE_RAW );
	$export = filter_input( INPUT_GET, 'export', FILTER_UNSAFE_RAW );
	if(empty($page) or $page !== 'mbt_import') { return; }
	if(empty($import_type) or $import_type !== 'mybooktable') { return; }
	if(empty($export) or ($export !== 'books' and $export !== 'books,settings')) { return; }
