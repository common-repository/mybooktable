...
function mbt_seo_init() {
	if(defined('WPSEO_FILE')) {
		...
		add_action('wp_head', 'mbt_seo_add_opengraph',999);
	}
...
function mbt_add_wpseo_opengraph_isbn() {
	...
	if(is_singular('mbt_book')) {
		...
		if(!empty($isbn)) { echo("<meta property='book:isbn' content='".esc_attr($isbn)."'/>\n"); }
	}
}
...
function mbt_seo_metadesc($post_id = 0) {
	...
	if($post) {
		...
		if(empty($seo_metadesc)) {
			$metadesc = wp_strip_all_tags($post->post_excerpt);
		} else {
...
function mbt_seo_add_metadesc() {
	...
	if(is_tax('mbt_author') or is_tax('mbt_series') or is_tax('mbt_genre') or is_tax('mbt_tag') or (mbt_is_archive_query() and $mbt_archive_query->is_tax())) {
		...
		if($metadesc) {
			echo('<meta name="description" content="'.esc_html($metadesc).'"/>'."\n");
		}
	} else if(is_singular('mbt_book')) {
...
function mbt_seo_add_opengraph() {
	...
	if(is_tax('mbt_author') or is_tax('mbt_series') or is_tax('mbt_genre') or is_tax('mbt_tag') or (mbt_is_archive_query() and $mbt_archive_query->is_tax())) {
		...
		$tags['og:url'] = apply_filters('mbt_get_ogurl', esc_url(get_permalink()));
		$tags['og:site_name'] = get_bloginfo('name');
		$image = mbt_get_book_image_src($post->ID);
	...
	foreach($tags as $tag => $content) {
		echo('<meta property="'.esc_html($tag).'" content="'.esc_attr($content).'"/>'.PHP_EOL);
	}
}
...
{
	 $sh_seo_nonce_field = wp_create_nonce('mbt-seo-nonce','data-seo-nonce');
...
?>
	...
	<table class="form-table mbt_seo_metabox">
		<tbody>
			<tr>
				<th scope="row">
					<label for="mbt_seo_title"><?php esc_attr_e('SEO Title:', 'mybooktable'); ?></label>
				</th>
				<td>
					<input type="hidden" id="data-seo-nonce" name="data-seo-nonce" value="<?php echo(esc_attr($sh_seo_nonce_field)); ?>">
					<input type="text" placeholder="" id="mbt_seo_title" name="mbt_seo_title" value="<?php echo(esc_attr(get_post_meta($post->ID, 'mbt_seo_title', true))); ?>" class="large-text"><br>
					<p><?php 
						/* translators: %s: number of characters */
						echo sprintf(esc_html__('Title display in search engines is limited to 70 characters, %s characters left.', 'mybooktable'), '<span id="mbt_seo_title-length">70</span>'); ?>
					</p>
				</td>
			</tr>
			<tr>
				<th scope="row"><label for="mbt_seo_metadesc"><?php esc_attr_e('Meta Description:', 'mybooktable'); ?></label></th>
				<td>
					<textarea class="large-text" rows="3" id="mbt_seo_metadesc" name="mbt_seo_metadesc"><?php echo(esc_attr(get_post_meta($post->ID, 'mbt_seo_metadesc', true))); ?></textarea>
					<p><?php 
						/* translators: %s: number of characters */
						echo sprintf(esc_html__('The meta description will be limited to 156 characters, %s characters left.', 'mybooktable'), '<span id="mbt_seo_metadesc-length">156</span>'); ?>
					</p>
				</td>
			</tr>
		</tbody>
...
function mbt_save_seo_metabox($post_id)
{
	...
	if( get_post_type($post_id) == "mbt_book" && ((isset($_REQUEST['data-seo-nonce']) && wp_verify_nonce(sanitize_key($_REQUEST['data-seo-nonce']),'mbt-seo-nonce')))) {
		
		if(isset($_REQUEST['mbt_seo_title'])) { update_post_meta($post_id, "mbt_seo_title", sanitize_text_field(wp_unslash($_REQUEST['mbt_seo_title']))); }
		if(isset($_REQUEST['mbt_seo_metadesc'])) { update_post_meta($post_id, "mbt_seo_metadesc", sanitize_text_field(wp_unslash($_REQUEST['mbt_seo_metadesc']))); }
	}
}
